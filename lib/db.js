...
export async function savePDFChunks(pdfUploadId, chunks, embeddings) {
  try {
    // Insert chunks using a single query if possible, or batch them
    // Neon's tagged template literal doesn't handle multidimensional arrays well for batch inserts
    // so we'll do individual inserts for now (can be optimized later)
    await Promise.all(chunks.map(async (content, i) => {
