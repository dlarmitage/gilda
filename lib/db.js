...
export async function searchPDFChunks(userId, queryEmbedding, limit = 5) {
  try {
    ...
    export async function createShareLink(shareId, data) {
      const { pdfContent, pdfMetadata, documents, userId, brandColor, brandTransparency } = data;

      await query`
    INSERT INTO share_links (
      id, pdf_content, pdf_metadata, documents, user_id, brand_color, brand_transparency
    ) VALUES (
      ${shareId}, 
      ${pdfContent}, 
