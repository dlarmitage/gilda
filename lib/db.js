...
export async function savePDFChunks(pdfUploadId, chunks, embeddings) {
  try {
    // Process in smaller serial batches to avoid overwhelming the connection pool/network
    // and hitting "fetch failed" errors on large documents.
    const batchSize = 10;
    ...
    for (let i = 0; i < chunks.length; i += batchSize) {
      ...
      await Promise.all(chunkBatch.map(async (content, j) => {
      ...
      // Small delay could be added here if needed, but sequential batches should be enough
    }
