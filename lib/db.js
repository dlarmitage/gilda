...
/**
 * Create a shares link
 */
export async function createShareLink(shareId, data) {
  const { pdfContent, pdfMetadata, documents, userId, brandColor, brandTransparency, publicTitle, publicDescription } = data;

  await query`
    INSERT INTO share_links (
      id, pdf_content, pdf_metadata, documents, user_id, brand_color, brand_transparency, public_title, public_description
    ) VALUES (
      ${shareId}, 
      ${pdfContent}, 
...
export async function getShareLink(shareId) {
  ...
  return {
    ...
    pdfMetadata: typeof data.pdf_metadata === 'string' ? JSON.parse(data.pdf_metadata) : data.pdf_metadata,
    documents: typeof data.documents === 'string' ? JSON.parse(data.documents) : data.documents,
    userId: data.user_id,
    brandColor: data.brand_color,
    brandTransparency: parseFloat(data.brand_transparency),
